...
{
    public class Battleship : BaseShip<Battleship>
    {
        public Battleship() : base(5, 1)
        {
        ...
        private List<List<Square>> Vertical(Tuple<char, int> currentPosition, List<Square> SquareBoard)
        {
            ...
            for (int i = 0; i < this.Size; i++)
            {
                ...
                if (SquareBoard.Any(r => r.Position.Equals(firstPoint) && (r.Status == PositionStatus.Empty || (r.Status == PositionStatus.Red && r.ShipType == ShipType.Battleship)))
                    && SquareBoard.Any(r => r.Position.Equals(firstPoint) && (r.Status == PositionStatus.Empty || (r.Status == PositionStatus.Red && r.ShipType == ShipType.Battleship))))
                {
                    ...
                    for (int j = firstPoint.Item2 - 1; j > lastPoint.Item2; j--)
                    {
                        ...
                        if (!SquareBoard.Any(r => r.Position.Equals(tempPoint) && (r.Status == PositionStatus.Empty || (r.Status == PositionStatus.Red && r.ShipType == ShipType.Battleship))))
                        {
                            tempList = null;
                            break;
                        }
        ...
        private List<List<Square>> Horizontal(Tuple<char, int> currentPosition, List<Square> SquareBoard)
        {
            ...
            for (int i = 0; i < this.Size; i++)
            {
                ...
                if (SquareBoard.Any(r => r.Position.Equals(firstPoint) && (r.Status == PositionStatus.Empty || (r.Status == PositionStatus.Red && r.ShipType == ShipType.Battleship)))
                    && SquareBoard.Any(r => r.Position.Equals(lastPoint) && (r.Status == PositionStatus.Empty || (r.Status == PositionStatus.Red && r.ShipType == ShipType.Battleship))))
                {
                    var tempList = new List<Tuple<char, int>>();
                    tempList.Add(firstPoint);
        ...
        {
            if (squares.Where(r => r.ShipType is ShipType.Battleship).Count() == 5)
            {
